...
# New constants for prioritized replay
PRIORITY_ALPHA_START = 0.6  # Start with a higher priority exponent
PRIORITY_ALPHA_END = 0.4    # End with a lower priority exponent
PRIORITY_BETA_START = 0.4   # Start with a lower importance sampling exponent
PRIORITY_BETA_END = 1.0     # End with a higher importance sampling exponent
...
class PrioritizedReplayBuffer:
    ...
    def push(self, *args):
        ...
        self.memory[self.position] = Transition(*args, priority=max_priority)
        self.priorities[self.position] = max_priority
        self.position = (self.position + 1) % self.capacity
        self.size = min(self.size + 1, self.capacity)

    def sample(self, batch_size, priority_alpha, priority_beta):
        if len(self.memory) == 0:
            return []
        
        priorities = self.priorities[:self.size] ** priority_alpha
        ...
        weights = (total * probabilities[indices]) ** (-priority_beta)
    ...
    def update_priorities(self, indices, priorities):
        for idx, priority in zip(indices, priorities):
            self.priorities[idx] = priority + EPSILON
...
def setup_training(self):
    ...
    self.replay_buffer = PrioritizedReplayBuffer(TRANSITION_HISTORY_SIZE)
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if self.random_value >= self.prob_record:
        self.replay_buffer.push(old_features, self_action, new_features, reward)
...
def enemy_game_events_occurred(self, enemy_name: str, old_enemy_game_state: dict, enemy_action: str, new_enemy_game_state: dict, enemy_events: List[str]):    
    ...
    if self.random_value < self.prob_record:
        ...
        self.replay_buffer.push(old_features, enemy_action, new_features, reward)

def train_prioritized(self, current_round):
    if len(self.replay_buffer.memory) < BATCH_SIZE:
        return
    
    # Calculate current priority exponent (alpha) and importance sampling exponent (beta)
    progress = min(1.0, current_round / N_TRAINING_EPSD)
    current_alpha = PRIORITY_ALPHA_START + progress * (PRIORITY_ALPHA_END - PRIORITY_ALPHA_START)
    current_beta = PRIORITY_BETA_START + progress * (PRIORITY_BETA_END - PRIORITY_BETA_START)
    
    # Sample batch
    transitions, indices, weights = self.replay_buffer.sample(BATCH_SIZE, current_alpha, current_beta)
    
    # Compute loss and update priorities
    new_priorities = []
    for transition, weight in zip(transitions, weights):
        state, action, next_state, reward = transition[:4]
        current_q = self.q_table[state][action]
        ...
        # Update Q-value
        self.q_table[state][action] += 0.1 * weight * (target_q - current_q)
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.replay_buffer.push(last_features, last_action, None, reward)
    self.train_prioritized(last_game_state['round'])
...
def reward_from_events(self, events: List[str]) -> int:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    game_rewards = {
        ...
        BOMB_PLACED_UNSAFELY: -80, 
        BOMB_PLACED_RISKILY: -7,   
    }
    reward_sum = 0
    for event in events:
