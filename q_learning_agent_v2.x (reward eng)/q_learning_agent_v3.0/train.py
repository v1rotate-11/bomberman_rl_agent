...
DID_NOT_MOVE_TOWARD_ENEMY = "DID_NOT_MOVE_TOWARD_ENEMY"
MOVED_AWAY_FROM_ENEMY = "MOVED_AWAY_FROM_ENEMY"
DID_NOT_MOVE_AWAY_FROM_ENEMY = "DID_NOT_MOVE_AWAY_FROM_ENEMY"
WAITED_WHEN_APPROPRIATE = "WAITED_WHEN_APPROPRIATE"
BOMB_PLACED_AT_OPTIMAL_POSITION = "BOMB_PLACED_AT_OPTIMAL_POSITION"
BOMB_PLACED_AT_USELESS_POSITION = "BOMB_PLACED_AT_USELESS_POSITION"
MOVED_TO_SAFETY = "MOVED_TO_SAFETY"
DID_NOT_MOVE_TO_SAFETY = "DID_NOT_MOVE_TO_SAFETY"
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    # Generate custom events based on the action taken
    custom_events = generate_events_from_action(old_game_state, self_action)


    # Combine game events with custom events
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if last_game_state['round'] == N_TRAINING_EPSD:
        self.tracker.save_current_session("q_learning_agent_v3.0")
...
def action_to_direction(action):
    if action == 'UP':
        ...
        return 0  # WAIT or BOMB

def get_highest_enemy_score(game_state):
    """Get the highest score among all enemies."""
    return max(enemy[1] for enemy in game_state['others']) if game_state['others'] else 0
...
def setup_rewards():
    game_rewards = {
        ...
        # Custom events
        MOVED_TOWARD_COIN: 3,
        DID_NOT_MOVE_TOWARD_COIN: -2,
        MOVED_TOWARD_CRATE: 1,
        DID_NOT_MOVE_TOWARD_CRATE: -0.5,
        MOVED_TOWARD_ENEMY: 1.5,
        DID_NOT_MOVE_TOWARD_ENEMY: -1,
        MOVED_AWAY_FROM_ENEMY: 3,
        DID_NOT_MOVE_AWAY_FROM_ENEMY: -2,
        MOVED_TO_SAFETY: 5,
        DID_NOT_MOVE_TO_SAFETY: -4,
        BOMB_PLACED_AT_OPTIMAL_POSITION: 3,
        WAITED_WHEN_APPROPRIATE: 0.5,
        BOMB_PLACED_AT_USELESS_POSITION: -3,

    }
    return game_rewards


def generate_events_from_action(old_game_state, action):
    old_features = state_to_features(old_game_state)
    suggested_direction, target_type = old_features[6]  # Unpack the tuple
    action_direction = action_to_direction(action)
    
    events = []
    
    if target_type == 'coin':
        if action_direction == suggested_direction:
            events.append(MOVED_TOWARD_COIN)
        else:
            events.append(DID_NOT_MOVE_TOWARD_COIN)
    elif target_type == 'crate':
        if action_direction == suggested_direction:
            events.append(MOVED_TOWARD_CRATE)
        else:
            events.append(DID_NOT_MOVE_TOWARD_CRATE)
    elif target_type == 'enemy':
        if action_direction == suggested_direction:
            events.append(MOVED_TOWARD_ENEMY)
        else:
            events.append(DID_NOT_MOVE_TOWARD_ENEMY)
    elif target_type == 'avoid_enemy':
        if action_direction != suggested_direction:
            events.append(MOVED_AWAY_FROM_ENEMY)
        else:
            events.append(DID_NOT_MOVE_AWAY_FROM_ENEMY)
    elif target_type == 'safety':
        if action_direction == suggested_direction:
            events.append(MOVED_TO_SAFETY)
        else:
            events.append(DID_NOT_MOVE_TO_SAFETY)
    elif target_type == 'wait':
        if action == 'WAIT':
            events.append(WAITED_WHEN_APPROPRIATE)
    
    # Add event for optimal bomb placement
    if old_features[-1] == 1 and action == 'BOMB':
        events.append(BOMB_PLACED_AT_OPTIMAL_POSITION)
    elif old_features[-1] != 1 and action == 'BOMB':
        events.append(BOMB_PLACED_AT_USELESS_POSITION)
    return events

def reward_from_events(self, events: List[str]) -> int:
    game_rewards = setup_rewards()
    reward_sum = 0
    for event in events:
