...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    old_coin_direction = old_features[6]  # Assuming coin direction is the 8th feature
    new_position = new_game_state['self'][3]
    old_position = old_game_state['self'][3]

    # Check if the agent moved in the suggested direction
    ...
    # Also keep the distance-based check
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    if last_game_state['round'] == N_TRAINING_EPSD:
        self.tracker.save_current_session("q_learning_agent_v2.0 (added coin direction rewards)")
