...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    def get_direction_to_nearest_enemy():
        if not others:
            return 0  # No enemies available

        # Find nearest enemies
        distances = [manhattan_distance(position, enemy[3]) for enemy in others]
        min_distance = min(distances)
        nearest_enemies = [enemy for enemy, dist in zip(others, distances) if dist == min_distance]

        if len(nearest_enemies) == 1:
            return get_direction(position, nearest_enemies[0][3])
        
        # Handle multiple equidistant enemies
        enemies_without_bombs = [enemy for enemy in nearest_enemies if not enemy[2]]
        if enemies_without_bombs:
            return get_direction(position, enemies_without_bombs[0][3])
        else:
            return get_direction(position, nearest_enemies[0][3])
    ...
    features.append(get_direction_to_nearest_coin())
    features.append(get_direction_to_nearest_enemy())
